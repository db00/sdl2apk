...
char * ByteArray_readLine(ByteArray * bytearray)
{
	...
	{
		int c = *(bytearray->data+bytearray->position+len);
		/*
...
ByteArray * ByteArray_sprint16(ByteArray*bytearray,int n,ByteArray * out)
{
	...
	if(ret){
		...
		//fprintf(stdout,"%s\n",ret);
		out = ByteArray_writeBytes(out,ret,strlen(ret));
		free(ret);
...
ByteArray * ByteArray_sprints(ByteArray*bytearray,int num,ByteArray * out)
{
	if(bytearray==NULL || bytearray->position>=bytearray->length)
		return out;
	...
	if(num>0 && num<0x1000)
	{
		...
		//fprintf(stdout,"%s",p);
		out = ByteArray_writeBytes(out,p,strlen(p));
	}
	return out;
}
...
ByteArray * ByteArray_writeBytes(ByteArray*bytearray,char*bytes,int length)
{
	if(length<=0 || bytes==NULL)
		return bytearray;
	if(bytearray==NULL)
		bytearray = ByteArray_new(length+1);
