/**
 * @file doc.c
 gcc -g doc.c ole.c bytearray.c -D test_doc -lm && ./a.out
 *  
 * @author db0@qq.com
 * @version 1.0.1
 * @date 2015-10-22
	...
	if(file->clx){
		...
		while(i<length-1)
		{
			...
			if(fc->A){//fCompressed
				...
				file->text = ByteArray_sprints(bytearray,end-start,file->text);
			}else{
				...
				file->text = ByteArray_sprint16(bytearray,(end-start)/2,file->text);
			}
			++i;
		}
	}
...
int main()
{
	...
	char * filename = "/home/db0/reader/RAR.doc";
