 ...
 * SDL1:
 gcc cmdutils.c ffplay2.c -lSDL  -I"/usr/include/ffmpeg/" -I"/usr/include/SDL/" -lm -l"avformat" -l"avcodec" -l"avutil" -l"swresample" -l"avfilter" -l"swscale" -l"avdevice" -lz && ./a.out ./a.flv
		...
		if (got_subtitle && sp->sub.format == 0) {
			...
			for (i = 0; i < sp->sub.num_rects; i++)
			{
				...
				is->sub_convert_ctx = sws_getCachedContext(is->sub_convert_ctx,
						in_w, in_h, AV_PIX_FMT_PAL8,
						out_w, out_h, AV_PIX_FMT_YUVA420P,
	...
	if (get_master_sync_type(is) != AV_SYNC_AUDIO_MASTER) {
		...
		if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD) {
			...
			if (is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
				...
				av_log(NULL, AV_LOG_TRACE, "diff=%f adiff=%f sample_diff=%d apts=%0.3f %f\r",
