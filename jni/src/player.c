/*
 * SDL2:
 gcc -g -Wall player.c -lSDL2 -I"/usr/include/ffmpeg/"  -I"/usr/include/SDL2/" -lm -l"avformat" -l"avcodec" -l"avutil" -l"swresample" -l"swscale"  && ./a.out
 gcc -g -Wall player.c -lSDL2 -I"/home/libiao/src/android-ndk-r10/sources/ffmpeg/android/arm/include/"  -I"../SDL2/include/" -lm -l"avformat" -l"avcodec" -l"avutil" -l"swresample"  && ./a.out a.flv
 */
#include <SDL.h>
#include <signal.h>
		...
		AVDictionary *codec_opts)
{
	...
	if (!opts) {
		SDL_Log( "Could not alloc memory for stream options.\n");
		return NULL;
	}
...
static int audio_decode_frame(VideoState *is)
{
	...
	if (is->swr_ctx) {
		...
		if (len2 == out_count) {
			SDL_Log("audio buffer is probably too small\n");
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
	...
	if (spec.format != AUDIO_S16SYS) {
		SDL_Log( "SDL advised audio format %d is not supported!\n", spec.format);
		return -1;
	}
	if (spec.channels != wanted_spec.channels) {
		wanted_channel_layout = av_get_default_channel_layout(spec.channels);
		if (!wanted_channel_layout) {
			SDL_Log( "SDL advised channel count %d is not supported!\n", spec.channels);
			return -1;
		}
	}
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	if(stream_lowres > av_codec_get_max_lowres(codec)){
		SDL_Log("The maximum value for lowres supported by the decoder is %d\n",
...
static int read_thread(void *arg)
{
	...
	if (start_time != AV_NOPTS_VALUE) {
		...
		if (ret < 0) {
			SDL_Log( "%s: could not seek to position %0.3f\n",
...
int main(int argc, char **argv)
{
	...
	if (SDL_Init (flags)) {
		...
		SDL_Log("(Did you set the DISPLAY variable?)\n");
		exit(1);
	}
	SDL_Rect r = {0};
