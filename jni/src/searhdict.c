...
static Word * _getWord(char * _word)
{
...
static int getMean(Word * word)
{
	...
	Explain_show(dictContainer,input->sprite->h,dict,word);
	return 0;
}


static int searchWord(char* _word)
{
	removePictures();
...
static Sprite * makeWordBtn(Word * word)
{
