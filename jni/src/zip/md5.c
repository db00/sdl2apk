...
static AS3_Val md5(void* self, AS3_Val args)
{

	AS3_Val string;
	AS3_ArrayValue(args, "AS3ValType", &string);
	...
	MD5Update(&md5,(unsigned char*)s,strlen((char *)s));
	MD5Final(&md5,out);
	...
	while(i < strlen(out))
	{
		sprintf(ret+strlen(ret),"%02x",out[i]);
		++i;
	}
	return AS3_String(ret);
}
...
int main()
{
	...
	AS3_Val md5Method = AS3_Function( NULL, md5);

	// construct an object that holds references to the functions
	AS3_Val result = AS3_Object( "md5: AS3ValType", md5Method );

	// Release
	AS3_Release( md5Method );
